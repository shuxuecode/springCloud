...
public class DubboHystrixCommand extends HystrixCommand<Result> {
	...
	protected DubboHystrixCommand(Invoker<?> invoker, Invocation invocation) {
	...
	protected Result getFallback() {
		if (executionResult.isResponseSemaphoreRejected()) {
			...
			map.put("resultCode", "错误码");
			map.put("resultMsg", "错误提示信息");

			Result result = new RpcResult(map);
			return result;
		}
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("resultCode", "公用错误码");
		map.put("resultMsg", "公用错误提示信息");

		
		Result result = new RpcResult(JSON.toJSONString(map));
		return result;
	}
